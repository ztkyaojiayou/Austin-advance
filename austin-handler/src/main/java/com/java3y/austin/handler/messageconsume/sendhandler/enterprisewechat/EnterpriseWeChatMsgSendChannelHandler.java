package com.java3y.austin.handler.messageconsume.sendhandler.enterprisewechat;
...
@Component
@Slf4j
public class EnterpriseWeChatMsgSendChannelHandler extends BaseMsgSendChannelHandler implements MsgSendChannelHandler {

    @Autowired
    private AccountUtils accountUtils;

    public EnterpriseWeChatMsgSendChannelHandler() {
    ...
    public boolean channel4MsgHandler(MessageInfo messageInfo) {
        try {
            WxCpDefaultConfigImpl accountConfig = accountUtils.getAccountById(messageInfo.getSendAccount(), WxCpDefaultConfigImpl.class);
            WxCpMessageServiceImpl messageService = new WxCpMessageServiceImpl(initService(accountConfig));
            WxCpMessageSendResult result = messageService.send(buildWxCpMessage(messageInfo, accountConfig.getAgentId()));
            if (Integer.valueOf(WxMpErrorMsgEnum.CODE_0.getCode()).equals(result.getErrCode())) {
                return true;
            }
            // 常见的错误 应当 关联至 AnchorState,由austin后台统一透出失败原因
            log.error("EnterpriseWeChatHandler#handler fail!result:{},params:{}", JSON.toJSONString(result), JSON.toJSONString(messageInfo));
        } catch (Exception e) {
     ...
     * @return
     */
    private WxCpMessage buildWxCpMessage(MessageInfo messageInfo, Integer agentId) {
        String userId;
        if (AustinConstant.SEND_ALL.equals(CollUtil.getFirst(messageInfo.getReceiver()))) {
            userId = CollUtil.getFirst(messageInfo.getReceiver());
        } else {
            userId = StringUtils.join(messageInfo.getReceiver(), CommonConstant.RADICAL);
        }
        EnterpriseWeChatContentModel contentModel = (EnterpriseWeChatContentModel) messageInfo.getContentModel();
