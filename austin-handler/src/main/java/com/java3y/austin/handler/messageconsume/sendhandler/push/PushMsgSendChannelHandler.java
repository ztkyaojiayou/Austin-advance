package com.java3y.austin.handler.messageconsume.sendhandler.push;
...
import com.java3y.austin.handler.common.domain.push.PushParam;
import com.java3y.austin.handler.common.domain.push.getui.BatchSendPushParam;
import com.java3y.austin.handler.common.domain.push.getui.SendPushParam;
import com.java3y.austin.handler.common.domain.push.getui.SendPushResult;
...
@Component
@Slf4j
public class PushMsgSendChannelHandler extends BaseMsgSendChannelHandler implements MsgSendChannelHandler {
    ...
    public PushMsgSendChannelHandler() {
    ...
    public boolean channel4MsgHandler(MessageInfo messageInfo) {
        ...
        try {
            GeTuiAccount account = accountUtils.getAccountById(messageInfo.getSendAccount(), GeTuiAccount.class);
            String token = redisTemplate.opsForValue().get(SendAccountConstant.GE_TUI_ACCESS_TOKEN_PREFIX + messageInfo.getSendAccount());
            PushParam pushParam = PushParam.builder().token(token).appId(account.getAppId()).messageInfo(messageInfo).build();

            String result;
            if (messageInfo.getReceiver().size() == 1) {
            ...
            log.error("PushHandler#handler fail!result:{},params:{}", JSON.toJSONString(sendPushResult), JSON.toJSONString(messageInfo));
        } catch (Exception e) {
            log.error("PushHandler#handler fail!e:{},params:{}", Throwables.getStackTraceAsString(e), JSON.toJSONString(messageInfo));
        }
        return false;
    }
    ...
    private String singlePush(PushParam pushParam) {
        ...
        SendPushParam sendPushParam = assembleParam((PushContentModel) pushParam.getMessageInfo().getContentModel(), pushParam.getMessageInfo().getReceiver());
    ...
    private String batchPush(String taskId, PushParam pushParam) {
        ...
        BatchSendPushParam batchSendPushParam = BatchSendPushParam.builder()
                .taskId(taskId)
                .isAsync(true)
                .audience(BatchSendPushParam.AudienceVO.builder().cid(pushParam.getMessageInfo().getReceiver()).build()).build();
        String body = HttpRequest.post(url).header(Header.CONTENT_TYPE.getValue(), ContentType.JSON.getValue())
                .header("token", pushParam.getToken())
    ...
    private String createTaskId(PushParam pushParam) {
        ...
        SendPushParam param = assembleParam((PushContentModel) pushParam.getMessageInfo().getContentModel());
        String taskId = "";
        try {
            String body = HttpRequest.post(url).header(Header.CONTENT_TYPE.getValue(), ContentType.JSON.getValue())
                    .header("token", pushParam.getToken())
            ...
            log.error("PushHandler#createTaskId fail :{},params:{}", Throwables.getStackTraceAsString(e), JSON.toJSONString(pushParam.getMessageInfo()));
        }
