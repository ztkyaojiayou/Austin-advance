package com.java3y.austin.handler.messageconsume.sendhandler.dingding;
...
@Slf4j
@Service
public class DingDingWorkNoticeMsgSendChannelHandler extends BaseMsgSendChannelHandler implements MsgSendChannelHandler {
    ...
    public DingDingWorkNoticeMsgSendChannelHandler() {
    ...
    public boolean channel4MsgHandler(MessageInfo messageInfo) {
        try {
            DingDingWorkNoticeAccount account = accountUtils.getAccountById(messageInfo.getSendAccount(), DingDingWorkNoticeAccount.class);
            OapiMessageCorpconversationAsyncsendV2Request request = assembleParam(account, messageInfo);
            String accessToken = redisTemplate.opsForValue().get(SendAccountConstant.DING_DING_ACCESS_TOKEN_PREFIX + messageInfo.getSendAccount());
            ...
            if (response.getErrcode() == 0) {
                redisTemplate.opsForList().leftPush(DING_DING_RECALL_KEY_PREFIX + messageInfo.getMessageTemplateId(), String.valueOf(response.getTaskId()));
                redisTemplate.expire(DING_DING_RECALL_KEY_PREFIX + messageInfo.getMessageTemplateId(), (DateUtil.endOfDay(new Date()).getTime() - DateUtil.current()) / 1000, TimeUnit.SECONDS);
                return true;
            }

            // 常见的错误 应当 关联至 AnchorState,由austin后台统一透出失败原因
            log.error("DingDingWorkNoticeHandler#handler fail!result:{},params:{}", JSON.toJSONString(response), JSON.toJSONString(messageInfo));
        } catch (Exception e) {
            log.error("DingDingWorkNoticeHandler#handler fail!{},params:{}", Throwables.getStackTraceAsString(e), messageInfo);
        }
        return false;
    }
     ...
     * @param messageInfo
     */
    private OapiMessageCorpconversationAsyncsendV2Request assembleParam(DingDingWorkNoticeAccount account, MessageInfo messageInfo) {
        OapiMessageCorpconversationAsyncsendV2Request req = new OapiMessageCorpconversationAsyncsendV2Request();
        DingDingWorkContentModel contentModel = (DingDingWorkContentModel) messageInfo.getContentModel();
        try {
            // 接收者相关
            if (AustinConstant.SEND_ALL.equals(CollUtil.getFirst(messageInfo.getReceiver()))) {
                req.setToAllUser(true);
            } else {
                req.setUseridList(StringUtils.join(messageInfo.getReceiver(), StrUtil.C_COMMA));
            }
            ...
            log.error("assembleParam fail:{},params:{}", Throwables.getStackTraceAsString(e), JSON.toJSONString(messageInfo));
        }
        return req;
    }
