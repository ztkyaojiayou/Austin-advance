package com.java3y.austin.handler.messageconsume.sendhandler.sms;
...
import com.java3y.austin.handler.common.domain.sms.MessageTypeSmsConfig;
import com.java3y.austin.handler.common.domain.sms.SmsParam;
...
import com.java3y.austin.handler.messagereceipt.SmsScript;
...
@Component
@Slf4j
public class SmsMsgSendChannelHandler extends BaseMsgSendChannelHandler implements MsgSendChannelHandler {

    public SmsMsgSendChannelHandler() {
    ...
    public boolean channel4MsgHandler(MessageInfo messageInfo) {
        SmsParam smsParam = SmsParam.builder()
                .phones(messageInfo.getReceiver())
                .content(getSmsContent(messageInfo))
        ...
        try {
             ...
             * 2、发送短信
             */
            MessageTypeSmsConfig[] messageTypeSmsConfigs = loadBalance(getMessageTypeSmsConfig(messageInfo));
     ...
     * @return
     */
    private List<MessageTypeSmsConfig> getMessageTypeSmsConfig(MessageInfo messageInfo) {

        /**
         * 如果模板指定了账号，则优先使用具体的账号进行发送
         */
        if (!messageInfo.getSendAccount().equals(AUTO_FLOW_RULE)) {
            SmsAccount account = accountUtils.getAccountById(messageInfo.getSendAccount(), SmsAccount.class);
            return Arrays.asList(MessageTypeSmsConfig.builder().sendAccount(messageInfo.getSendAccount()).scriptName(account.getScriptName()).weights(100).build());
        }
        ...
        for (int i = 0; i < jsonArray.size(); i++) {
            JSONArray array = jsonArray.getJSONObject(i).getJSONArray(FLOW_KEY_PREFIX + messageInfo.getMsgType());
