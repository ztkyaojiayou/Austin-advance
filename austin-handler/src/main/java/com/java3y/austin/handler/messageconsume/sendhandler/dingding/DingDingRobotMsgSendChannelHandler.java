...
import com.java3y.austin.handler.common.domain.dingding.DingDingRobotParam;
import com.java3y.austin.handler.common.domain.dingding.DingDingRobotResult;
...
@Slf4j
@Service
public class DingDingRobotMsgSendChannelHandler extends BaseMsgSendChannelHandler implements MsgSendChannelHandler {

    @Autowired
    private AccountUtils accountUtils;

    public DingDingRobotMsgSendChannelHandler() {
    ...
    public boolean channel4MsgHandler(MessageInfo messageInfo) {
        try {
            DingDingRobotAccount account = accountUtils.getAccountById(messageInfo.getSendAccount(), DingDingRobotAccount.class);
            DingDingRobotParam dingDingRobotParam = assembleParam(messageInfo);
            ...
            log.error("DingDingHandler#handler fail!result:{},params:{}", JSON.toJSONString(dingDingRobotResult), JSON.toJSONString(messageInfo));
        } catch (Exception e) {
            log.error("DingDingHandler#handler fail!e:{},params:{}", Throwables.getStackTraceAsString(e), JSON.toJSONString(messageInfo));
        }
        return false;
    }


    private DingDingRobotParam assembleParam(MessageInfo messageInfo) {
        ...
        // 消息类型以及内容相关
        DingDingRobotContentModel contentModel = (DingDingRobotContentModel) messageInfo.getContentModel();
