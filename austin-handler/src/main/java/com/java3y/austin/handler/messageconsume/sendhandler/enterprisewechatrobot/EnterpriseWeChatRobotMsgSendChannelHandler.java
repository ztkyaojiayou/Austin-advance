package com.java3y.austin.handler.messageconsume.sendhandler.enterprisewechatrobot;
...
import com.java3y.austin.handler.common.domain.wechat.robot.EnterpriseWeChatRobotParam;
...
@Slf4j
@Service
public class EnterpriseWeChatRobotMsgSendChannelHandler extends BaseMsgSendChannelHandler implements MsgSendChannelHandler {

    @Autowired
    private AccountUtils accountUtils;

    public EnterpriseWeChatRobotMsgSendChannelHandler() {
    ...
    public boolean channel4MsgHandler(MessageInfo messageInfo) {
        try {
            EnterpriseWeChatRobotAccount account = accountUtils.getAccountById(messageInfo.getSendAccount(), EnterpriseWeChatRobotAccount.class);
            EnterpriseWeChatRobotParam enterpriseWeChatRobotParam = assembleParam(messageInfo);
            ...
            log.error("EnterpriseWeChatRobotHandler#handler fail! result:{},params:{}", JSON.toJSONString(weChatRootResult), JSON.toJSONString(messageInfo));
        } catch (Exception e) {
            log.error("EnterpriseWeChatRobotHandler#handler fail!e:{},params:{}", Throwables.getStackTraceAsString(e), JSON.toJSONString(messageInfo));
        }
        return false;
    }

    private EnterpriseWeChatRobotParam assembleParam(MessageInfo messageInfo) {
        EnterpriseWeChatRobotContentModel contentModel = (EnterpriseWeChatRobotContentModel) messageInfo.getContentModel();
