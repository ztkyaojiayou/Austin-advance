package com.java3y.austin.handler.messageconsume.sendhandler.wxofficial;
...
@Component
@Slf4j
public class OfficialAccountMsgSendChannelHandler extends BaseMsgSendChannelHandler implements MsgSendChannelHandler {

    @Autowired
    private AccountUtils accountUtils;

    public OfficialAccountMsgSendChannelHandler() {
    ...
    public boolean channel4MsgHandler(MessageInfo messageInfo) {
        try {
            OfficialAccountsContentModel contentModel = (OfficialAccountsContentModel) messageInfo.getContentModel();
            WxMpService wxMpService = accountUtils.getAccountById(messageInfo.getSendAccount(), WxMpService.class);
            List<WxMpTemplateMessage> messages = assembleReq(messageInfo.getReceiver(), contentModel);
            for (WxMpTemplateMessage message : messages) {
                try {
                    wxMpService.getTemplateMsgService().sendTemplateMsg(message);
                } catch (Exception e) {
                    log.info("OfficialAccountHandler#handler fail! param:{},e:{}", JSON.toJSONString(messageInfo), Throwables.getStackTraceAsString(e));
                }
            }
            return true;
        } catch (Exception e) {
            log.error("OfficialAccountHandler#handler fail:{},params:{}", Throwables.getStackTraceAsString(e), JSON.toJSONString(messageInfo));
        }
        return false;
    }
