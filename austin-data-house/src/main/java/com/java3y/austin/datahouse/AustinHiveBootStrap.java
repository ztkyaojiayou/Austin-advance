...
public class AustinHiveBootStrap {
  ...
  public static void main(String[] args) {
    ...
    HiveCatalog catalog =
        new HiveCatalog(catalogName, DataHouseConstant.CATALOG_DEFAULT_DATABASE, null);
    tableEnv.registerCatalog(catalogName, catalog);
    tableEnv.useCatalog(catalogName);

    // 2.创建Kafka源表
    String kafkaSourceTableCreate =
        "DROP TABLE IF EXISTS "
            + DataHouseConstant.CATALOG_DEFAULT_DATABASE
            + "."
    ...
    tableEnv.executeSql(
        "INSERT INTO "
            ...
            + "."
            + DataHouseConstant.KAFKA_SINK_TABLE_NAME
            + " SELECT ids,state,businessId,logTimestamp FROM "
            + DataHouseConstant.CATALOG_DEFAULT_DATABASE
            + "."
            + DataHouseConstant.KAFKA_SOURCE_TABLE_NAME
            + "");
