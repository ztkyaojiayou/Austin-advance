...
public class DataServiceImpl implements DataService {
  ...
  public UserTimeLineVo getTraceUserInfo(String receiver) {
    ...
    for (Map.Entry<String, List<SimpleAnchorInfo>> entry : map.entrySet()) {
      ...
      for (SimpleAnchorInfo simpleAnchorInfo : entry.getValue()) {
        ...
        String startTime =
            DateUtil.format(
                new Date(simpleAnchorInfo.getTimestamp()), DatePattern.NORM_DATETIME_PATTERN);
        ...
        if (StrUtil.isNotBlank(detail)) {
          UserTimeLineVo.ItemsVO itemsVO =
              UserTimeLineVo.ItemsVO.builder()
                  ...
                  .sendType(
                      ChannelType.getEnumByCode(messageTemplate.getSendChannel()).getDescription())
  ...
  public SmsTimeLineVo getTraceSmsInfo(DataParam dataParam) {
    ...
    Integer sendDate =
        Integer.valueOf(
            DateUtil.format(
                new Date(dataParam.getDateTime() * 1000L), DatePattern.PURE_DATE_PATTERN));
